    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final') {
            ...
            if ($il->show_previous_bal == 1) {
                ...
                $total_ledger_discount = Transaction::where('contact_id',$transaction->contact_id)
                    ->where('business_id', $transaction->business_id)
                    ->where('status', '!=', 'draft')
                    ->where('type','=', 'ledger_discount')
                    ->sum('final_total');
                
                $all_due -= (int)$total_ledger_discount;
    ...
    */
    public function getListSells($business_id, $sale_type = 'sell')
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ...
                ->leftJoin('users as u', 'transactions.created_by', '=', 'u.id')
                  ->leftJoin('users as comission_user', 'transactions.commission_agent', '=', 'comission_user.id')
                ...
                ->select(
                  ...
                  DB::raw("CONCAT(COALESCE(comission_user.surname, ''),' ',COALESCE(comission_user.first_name, ''),' ',COALESCE(comission_user.last_name,'')) as comission_user_name"),
                );
