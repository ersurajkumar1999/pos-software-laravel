...
class ProductUtil extends Util
{
     ...
     * @return array
     */
    public function getDetailsFromVariation($variation_id, $business_id, $location_id = null, $check_qty = true, $is_quotation= false)
    {
        ...
        if ($check_qty) {
            if($is_quotation == false || $is_quotation == 'false' || $is_quotation == "false"){
        ...
        if (!empty($location_id) && $check_qty) {
            ...
            if($is_quotation == false || $is_quotation == 'false' || $is_quotation == "false"){
                // dd($is_quotation, $variation_id, $business_id, $location_id);
            $query->where(function ($query) use ($location_id) {
                $query->where('p.enable_stock', '!=', 1)
    ...
    public function calculateInvoiceTotal($products, $tax_id, $discount = null, $uf_number = true)
    {
        ...
        foreach ($products as $product) {
            ...
            if (!empty($tax_id)) {
                ...
                $discounted_amount = $unit_price - $d_amount;
                if (!empty($tax_details)) {
                    if($product_tax){
                        $tax_amount += ($tax_details->amount/100) * ($discounted_amount)*$product_quantity;
                    }
                }
            }
        }
