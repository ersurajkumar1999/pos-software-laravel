...
class AdminSidebarMenu
{
 ...
 //                        $menu->url(action('Install\ModulesController@index'), __('lang_v1.modules'), ['icon' => 'fa fas fa-plug', 'active' => request()->segment(1) == 'manage-modules']//)->order(60);
  //                   }
            ...
            if (request()->session()->get('business.enable_price_screen') == 1){
                $menu->url(action('HomeController@findPrice'), 'Price Screen',
                    ['icon' => 'fa fas fa-cubes', 'active' => request()->segment(1) == 'find-price'])->order(86);
            }


            if (in_array('packages', $enabled_modules)) {

                $menu->url(route('packages'), 'Packages',
                    ['icon' => 'fa fas fa-box', 'active' => request()->segment(1) == 'packages'])->order(87);
            }
            ...
            if (auth()->user()->can('business_settings.access') ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules) {
                        ...
                        if(config('constants.reset_database')){
                            $sub->url(action('BusinessController@reset_database'),'Reset Database', ['icon' => 'fa fas fa-database', 'active' => request()->segment(1) == 'reset-database'])->order(86);
                        }
