    ...
    public function getCustomerSuppliers(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($contacts)
                ...
                ->addColumn('due', function ($row) use ($business_id) {
                    ...
                    $total_ledger_discount = Transaction::where('contact_id', $row->id)
                    ...
                    if ($row->contact_type == 'supplier') {
                        ...
                        $due += $row->opening_balance - $row->opening_balance_paid - $total_ledger_discount;
                    }
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionSellLine::join(
                ...
                ->select(
                    ...
                    DB::raw('ROUND(((transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax) - ((transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * v.dpp_inc_tax), 2) as grossprofit'),    
            	)
            ...
            return Datatables::of($query)
            	...
            	->editColumn('goods_price', function ($row) {
                     //ignore child sell line of combo product
                     $class = is_null($row->parent_sell_line_id) ? 'goods_price' : '';

                     return '<span class="'.$class.'"  data-orig-value="'.$row->goods_price.'">'.
                    $this->transactionUtil->num_f($row->goods_price, true).'</span>';
                 })
                 ->editColumn('subtotal', function ($row) {
                    //ignore child sell line of combo product
            	...
            	->editColumn('grossprofit', function ($row) {
                    //ignore child sell line of combo product
                    $class = is_null($row->parent_sell_line_id) ? 'grossprofit' : '';

                    return '<span class="'.$class.'"  data-orig-value="'.$row->grossprofit.'">'.
                    $this->transactionUtil->num_f($row->grossprofit, true).'</span>';
                })
                ...
                ->rawColumns(['invoice_no', 'unit_sale_price', 'goods_price', 'grossprofit' , 'subtotal', 'sell_qty', 'discount_amount', 'unit_price', 'tax', 'customer'])
                ->make(true);
        }
