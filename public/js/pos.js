$(document).ready(function() {
        ...
        $('#search_product')
            .autocomplete({
                ...
                select: function(event, ui) {
                    console.log("ui.item.enable_stock select",);
                    ...
                    if (ui.item.enable_stock != 1 || ui.item.qty_available > 0 || is_overselling_allowed || for_so) {
                        ...
                        pos_product_row(ui.item.variation_id, purchase_line_id,null,'',ui.item.weight_scale);
                    } else {
                        console.log("ui.item.enable_stock else",is_quotation);
                        if(is_quotation){
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function() {
        ...
        var unit_price_inc_tax = __read_number(tr.find('input.input_number'));
    ...
    $('table#pos_table tbody').on('change', 'input.pos_unit_price_inc_tax', function() {
        ...
        console.log("discounted_unit_price==>",discounted_unit_price);
    ...
    $('table#pos_table tbody').on(
        ...
        function() {
            ...
            var unit_price_inc_tax = __add_percent(discounted_unit_price, tax_rate);
            var line_total = quantity * unit_price_inc_tax;
            var this_total_unit_price = quantity * this_unit_price;
            console.log("discounted_unit_price==>",discounted_unit_price);
            console.log("quantity==>",quantity);
            console.log("unit_price_inc_tax==>",unit_price_inc_tax);
            console.log("discounted_unit_price==>",discounted_unit_price);
            console.log("line_total=>",line_total);
            console.log("this_total_unit_price==>",this_total_unit_price);

            // __write_number(tr.find('input.pos_unit_price_inc_tax'), unit_price_inc_tax);
            __write_number(tr.find('input.pos_line_total'), this_total_unit_price, false, 2);
            tr.find('span.pos_line_total_text').text(__currency_trans_from_en(this_total_unit_price, true));
            pos_each_row(tr);
            pos_total_row();
            round_row_to_iraqi_dinnar(tr);
        }
    ...
    $('button#pos-cancel').click(function() {
        swal({
            ...
            if (confirm) {
                $('.is_quotation').prop('checked', false).css('display', 'none').css('display', 'block');
    ...
    $('select#product_category, select#product_brand, select#select_location_id').on('change', function(e) {
       ...
       $( ".product_category_block" ).click(function() {
        $( ".product_category_block" ).each(function( index ) {
            let  this_func =   $(this);
            if (this_func.hasClass('btn-success')){
                this_func.removeClass('btn-success')
                if (!this_func.hasClass('btn-primary')){
                    this_func.addClass('btn-primary')
                }
            }
        });
        ...
        const location_id = $('input#location_id').val();
        if (location_id != '' || location_id != undefined) {
            get_product_suggestion_list(
                category_id,
                $('select#product_brand').val(),
...
function pos_product_row(variation_id = null, purchase_line_id = null, weighing_scale_barcode = null, quantity = 1,weight_scale =null) {
...
function pos_each_row(row_obj) {
    ...
    if (discount > 0) {
        ...
        __write_number(row_obj.find('input.pos_line_total'), discounted_unit_price);
    }
    console.log("unit_price==>",unit_price);
    console.log("discounted_unit_price==>",discounted_unit_price);
    console.log("unit_price_inc_tax==>",unit_price_inc_tax);
...
function calculate_billing_details(price_total) {
    ...
    var amount_discount_with_order_tax = pos_order_tax();
    ...
    var total_payable = amount_discount_with_order_tax + shipping_charges + packing_charge + additional_expense;
...
function pos_order_tax_old(price_total, discount) {
...
function pos_order_tax(price_total, discount) {
    ...
    if ($('#reward_point_enabled').length) {
        total_customer_reward = $('#rp_redeemed_amount').val();
        total_discount = parseFloat(total_discount) + parseFloat(total_customer_reward);
...
$(document).on('change', 'select#customer_id', function(){
    ...
    if ($(this).val() == default_customer_id) {
        ...
        if ($('#enable_packages_module').length) {
...
function show_package_products(){
    var customer_id = $('#customer_id').val();
    $.ajax({
        method: 'POST',
        url: '/get-package-detail',
        data:{
          'customer_id' : customer_id
        },
        success: function(result) {
            console.log(result);
            if (result.status){
                $('#packages_products_body').html(result.html);
                $('#packages_products_modal').modal('show');
            }
        },
    });
}
