$(document).ready(function() {
    ...
    if ($('#sr_date_filter').length == 1) {
        ...
        sr_sales_commission_report = $('table#sr_sales_with_commission_table').DataTable({
            ...
            fnDrawCallback: function(oSettings) {
                ...
                $('#footer_shipping_charges_total').text(
                    sum_table_col($('#sr_sales_with_commission_table'), 'shipping_charges')
                );
    ...
    product_sell_report = $('table#product_sell_report_table').DataTable({
        ...
        columns: [
            ...
            { data: 'subtotal', name: 'subtotal', searchable: false },
            { data: 'grossprofit', name: 'grossprofit', searchable: false },
        ],
        fnDrawCallback: function(oSettings) {
        
           	var totalSubtotal = sum_table_col($('#product_sell_report_table'), 'row_subtotal');
            $('#footer_subtotal').text(totalSubtotal);
            
            var totalGrossProfit = sum_table_col($('#product_sell_report_table'), 'grossprofit');
            $('#footer_grossprofit').text(totalGrossProfit.toFixed(2));

            var totalGrossProfit = sum_table_col($('#product_sell_report_table'), 'goods_price');
            $('#footer_goodsprice').text(totalGrossProfit.toFixed(2));            
